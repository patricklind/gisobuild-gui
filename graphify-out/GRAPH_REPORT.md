...
1. `GisoWebTests` - 213 edges
2. `PlatformCompatibilityTests` - 35 edges
3. `create_build_plan()` - 29 edges
4. `validate_smu_selection()` - 28 edges
5. `OperatorFlowTests` - 27 edges
...
Cohesion: 0.07
Nodes (22): platforms(), _bundle_files_by_csc(), capabilities_for_platform(), check_upgrade_matrix(), matrix_platform(), GisoBuildCapabilities, infer_platform(), is_lnt_platform() (+14 more)
...
### Community 47 - "app.py"
Cohesion: 0.07
Nodes (44): errorhandler, get, activity(), archive_checksums(), archive_delete(), archive_download(), archive_list(), bad_request() (+36 more)

### Community 48 - "log_event"
Cohesion: 0.11
Nodes (33): after_request, Exception, append_activity(), build_environment_blockers(), build_plan(), cancel_upload(), cisco_accept(), cisco_client() (+25 more)

### Community 49 - "normalize_architecture"
Cohesion: 0.50
Nodes (4): iso_architectures_from_listing(), Fall back to the ISO's own RPM repository when no eXR metadata file exists. LNT…, normalize_architecture(), Return the canonical processor family for a filename/metadata token.

### Community 52 - "run_job"
Cohesion: 0.09
Nodes (26): archive_giso_artifacts_and_cleanup(), build_report(), BuildCancelled, builder_process_environment(), cancellation_requested(), child_mount_args(), file_sha256(), giso_artifact_candidates() (+18 more)
...
### Community 57 - "upload_complete"
Cohesion: 0.31
Nodes (9): delete, archive_suffix(), delete_upload(), extract_cisco_archive(), extraction_path(), The archive suffix a filename ends with (case-insensitive), or None., (stem, suffix) that keeps a multi-part archive suffix like ".tar.gz" intact., split_upload_name() (+1 more)

### Community 59 - "enforce_archive_policy"
Cohesion: 0.17
Nodes (11): archive_size(), archive_timestamp(), build_space_blockers(), build_volume_free_bytes(), enforce_archive_policy(), Free space on every volume a build actually touches, not just uploads. Every…, Name every build volume that cannot fit this build, for the pre-build gate.…, Remove expired archive jobs, then oldest jobs until the archive fits its quota. (+3 more)

### Community 60 - "add_superseded_exclusions"
Cohesion: 0.15
Nodes (17): add_superseded_exclusions(), explain_with_prerequisites(), A package's containing directory names the Cisco supersedence identifier it…, Explain, rather than silently drop, RPMs active_rpm_names() already filtered…, Every bounded-size .txt in the workspace - Cisco ships one README per SMU., {SMU name: {rpm basename: md5}} from each Cisco SMU README's "RPMS:" block.…, {SMU name: {package name: prerequisite SMU name}} from each README. A Cisco SMU…, Name the Cisco SMU that supplies each unsatisfiable requirement, when a README… (+9 more)

### Community 61 - "persist_job"
Cohesion: 0.12
Nodes (16): before_request, append_log(), cancel_job(), expire_upload_sessions(), initialize_job_store(), log_startup_self_test_once(), persist_job(), Everything a build depends on that can be checked without starting one. Each… (+8 more)

### Community 62 - "Path"
Cohesion: 0.07
Nodes (56): active_rpm_names(), archive_source_for_extraction(), build_cleanup_paths(), build_command(), cisco_text(), compatibility(), confidence_report(), create_build_plan() (+48 more)

## Knowledge Gaps
- **237 isolated node(s):** `inputs`, `platformProfiles`, `VOLUME_LABELS`, `CONFIDENCE_LABELS`, `READY_CHECK_LABELS` (+232 more)
...
- **Why does `GisoWebTests` connect `GisoWebTests` to `.test_build_waits_for_tar_extraction_to_finish`, `PlatformCompatibilityTests`, `patch`, `IsoArchitectureInspectionTests`, `.test_discover_tolerates_file_removed_during_scan`, `._smu_fix`, `.upload`, `dict`, `.test_run_job_honors_cancellation_during_image_pull`, `Path`, `.test_inventory_reports_where_each_rpm_identity_comes_from`?**
  _High betweenness centrality (0.205) - this node is a cross-community bridge._
- **Why does `validate_smu_selection()` connect `PlatformCompatibilityTests` to `normalize_architecture`, `IsoArchitectureInspectionTests`, `Path`, `app.py`?**
