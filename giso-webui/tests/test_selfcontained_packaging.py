...
class SelfContainedPackagingTests(unittest.TestCase):
    ...
    def test_image_pins_one_exact_gisobuild_commit_and_uses_the_local_runner(self):
        commits = set(
            re.findall(r"^ARG GISOBUILD_COMMIT=(\S+)$", self.dockerfile, re.MULTILINE)
        )
        self.assertEqual(len(commits), 1, commits)
        self.assertRegex(commits.pop(), r"^[0-9a-f]{40}$")
        self.assertIn(
            'test "$(git rev-parse HEAD)" = "$GISOBUILD_COMMIT"', self.dockerfile
        )
    ...
    def test_image_verifies_and_records_one_sha256_of_the_gisobuild_source(self):
        pins = set(
            re.findall(
                r"^ARG GISOBUILD_SOURCE_SHA256=(\S+)$", self.dockerfile, re.MULTILINE
            )
        )
        ...
        self.assertIn(
            "GISOBUILD_SOURCE_MANIFEST=/opt/gisobuild.sha256sums", self.dockerfile
        )

    def test_default_compose_builds_our_image_and_the_socket_variant_is_explicit_and_pinned(
        self,
    ):
        ...
        self.assertEqual(
            self.compose.count("image: ${GISO_WEBUI_IMAGE:-giso-webui-selfcontained}"),
            2,
        )
        ...
        self.assertRegex(
            images[0], r"^ciscogisobuild/cisco-xr-gisobuild:[\w.]+@sha256:[0-9a-f]{64}$"
        )
    ...
    def test_env_example_sets_nothing_that_only_the_socket_deployment_uses(self):
        ...
        for name in active:
            self.assertIn(
                f"${{{name}",
                self.compose,
                f"{name} in .env.example is not used by compose.yaml",
            )
